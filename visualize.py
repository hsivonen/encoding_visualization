...
def format_index(name, row_length, lang):
    ...
    for code_point in indexes[name]:
        ...
        elif previous and previous + 1 == code_point:
            out_file.write((u"<td class='contiguous %s' title='U+%04X, %d = 0x%X'>%s" % (classify(code_point), code_point, pointer, pointer, unichr(code_point))).encode("utf-8"))
        else:
            out_file.write((u"<td class='discontiguous %s' title='U+%04X, %d = 0x%X'>%s" % (classify(code_point), code_point, pointer, pointer, unichr(code_point))).encode("utf-8"))
        previous = code_point
        pointer += 1
...
index_file.write("""<!DOCTYPE html><html lang=en><meta charset=utf-8><title>Encoding Visualizations</title><link rel=stylesheet href=visualization.css type=text/css>
    ...
    <li class=upper>Three bytes in UTF-8
    <li class="upper contiguous">Three bytes in UTF-8, code point follows immediately the code point of previous pointer
